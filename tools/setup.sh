...
if [[ ! "$GIT_ROOT" =~ /Kustom-Alarm$ ]]; then
  echo -e "${RED}ERROR:${NC} Please run this script from the cloned Kustom-Alarm directory."
  exit 1
fi
...
Please import the code style settings in Android Studio:
  ...
  * find the file ${GIT_ROOT}/tools/kustomalarm-codestyle.xml
